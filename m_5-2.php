...
//テーブル作成
$sql = "CREATE TABLE IF NOT EXISTS missionFIVE"
    ."("
...
if (isset($_POST["EDIT"],$_POST["edipass"])){
    ...
    if($passChecker>0){
       //データレコードの抽出
       $sql ='SELECT * FROM missionFIVE WHERE id= '.$edit.'';
       $stmt = $pdo->query($sql);
       $results = $stmt->fetchAll();
       foreach ($results as $row){
        //フォームに表示するデータ
...
if(isset($_POST["submit"],$_POST["pass"])){
    //a.編集
      if($_POST["editt"]!=0){
             //データレコードの更新
             $id=$_POST["editt"];
             $sql = 'UPDATE missionFIVE SET name=:name,comment=:comment WHERE id=:id';
    ...
    //データレコードの挿入
    $sql = $pdo -> prepare("INSERT INTO missionFIVE (name, comment, date, pass) VALUES (:name, :comment, :date, :pass)");
